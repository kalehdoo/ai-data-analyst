...
export default function ClaudeResponse({ prompt, onClose }: Props) {
      ...
      <div style={s.modal}>
        ...
        <div style={s.header}>
          <div style={s.headerLeft}>
            <span style={s.claudeIcon}>✦</span>
            <span style={s.headerTitle}>Claude Analysis</span>
  ...
  <div style={s.promptText}>{prompt}</div>
        ...
        {!started && (
          <div style={s.sendArea}>
            <button onClick={sendToClause} style={s.sendBtn}>
              ✦ Send to Claude
            </button>
            <p style={s.sendHint}>
              Claude will use your MCP tools to query the database and generate a full analysis.
            </p>
          </div>
        )}
...
// Simple markdown-like renderer for code blocks and text
        ...
        return (
          <div key={i} style={md.text}>
            {part.split("\n").map((line, j) => {
              ...
              if (line.startsWith("**") && line.endsWith("**")) return <strong key={j} style={md.bold}>{line.slice(2, -2)}</strong>;
...
const s: Record<string, React.CSSProperties> = {
  ...
  claudeIcon: { fontSize: 18, color: "var(--accent)" },
...
promptText: {
  ...
  sendArea: { padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 },
  sendBtn: {
  ...
  sendHint: { fontSize: 12, color: "var(--text-muted)", textAlign: "center" as const, maxWidth: 400 },
  errorBox: {
...
const md: Record<string, React.CSSProperties> = {
  ...
  bold: { color: "var(--text-primary)", display: "block", marginBottom: 4 },
  text: { marginBottom: 8 },
  codeBlock: {
  ...
  code: {
    ...
    color: "var(--accent)", whiteSpace: "pre-wrap" as const, overflowX: "auto" as const,
  },
};